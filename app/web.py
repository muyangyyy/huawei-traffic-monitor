...
const defaultCustomDual={in:".1.3.6.1.2.1.31.1.1.1.6.45",out:".1.3.6.1.2.1.31.1.1.1.10.45"};
...
function deviceDefaults(){return{id:"",name:"",host:"",snmp_version:"2c",community:"",port:161,oid_profile:"if_mib_64",custom_oid:"",custom_direction:"in",custom_in_oid:defaultCustomDual.in,custom_out_oid:defaultCustomDual.out,monitor_interfaces:[],enabled:true,mock:false}}
...
function updateOidFields(node){const profile=q(node,"[data-field=oid_profile]").value;all(node,".custom-single-field").forEach(el=>{el.style.display=profile==="custom_single"?"block":"none"});all(node,".custom-dual-field").forEach(el=>{el.style.display=profile==="custom_dual"?"block":"none"});if(profile==="custom_dual"){const inOid=q(node,"[data-field=custom_in_oid]"),outOid=q(node,"[data-field=custom_out_oid]");if(!inOid.value.trim())inOid.value=defaultCustomDual.in;if(!outOid.value.trim())outOid.value=defaultCustomDual.out}}
